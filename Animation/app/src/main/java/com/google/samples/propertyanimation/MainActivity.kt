...
class MainActivity : AppCompatActivity() {
    ...
    private fun shower() {
        ...
        // random size, from .1x to 1.6x of its default size
        newStar.scaleX = Math.random().toFloat() * 1.5f + .1f
        newStar.scaleY = newStar.scaleX
        starW = newStar.scaleX
        starH = newStar.scaleY

        // randomly somewhere from the left edge to the right edge
        newStar.translationX = Math.random().toFloat() * containerW - starW / 2
        // create two animators and add an interpolator to each
        val mover = ObjectAnimator.ofFloat(newStar, View.TRANSLATION_Y, -starH, containerH + starH)
        mover.interpolator = AccelerateInterpolator(1f)
        // star will rotate a random amount between 0 and 1080 degrees (three times around)
        val rotator = ObjectAnimator.ofFloat(newStar, View.ROTATION, (Math.random() * 1080).toFloat())
        rotator.interpolator = LinearInterpolator()

        val set = AnimatorSet()
        set.playTogether(mover, rotator)
        // random number between 500 and 2000 milliseconds
